...
class MultiHeadAttention(layers.Layer):
    ...
    def call(self, query, values):
        # query: (batch, hidden) -> need to reshape to (batch, 1, hidden)
        ...
        # To get attention weights, we need to call attention again
        _, attention_weights = self.attention(
            query=query,
            value=values,
            return_attention_scores=True
        )  # attention_weights: (batch, num_heads, 1, time)
        
        # Average attention weights across heads
        attention_weights = tf.reduce_mean(attention_weights, axis=1)  # (batch, 1, time)
...
history = tts_model.fit(
    ...
    callbacks=[
        ...
        tf.keras.callbacks.ModelCheckpoint('./models/best_mh_model.keras', save_best_only=True)
    ]
)
